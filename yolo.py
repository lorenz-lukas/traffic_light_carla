...
class TinyYoloFeature:
    ...
    def __init__(self, input_size, config):
        ...
        ###### CHANGES TO KERAS 2.4.3 AND TF 2.3.0 
...
class YOLO(object):
    def __init__(self, config):
        ...
        self.optimizer = SGD(lr=1e-5, momentum=0.9, decay=0.0005)
    ...
    def train(self):
        ...
        # history = self.model.fit(generator=train_generator,
        #                         steps_per_epoch=len(train_generator),
        #                         epochs=self.config['train']['nb_epochs'],
        #                         verbose=1,
        #                         validation_data=validation_generator,
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        cell_x = tf.cast(
            (tf.reshape(tf.tile(tf.range(self.grid_w), [self.grid_h]), (1, self.grid_h, self.grid_w, 1, 1)) ), dtype=tf.float32)
        ...
        conf_mask_neg = tf.cast( (best_ious < 0.50) ,dtype=tf.float32) * (1 - y_true[..., 4]) * self.no_object_scale
        ...
        nb_coord_box = tf.reduce_sum(tf.cast(conf_mask > 0.0, dtype=tf.float32))
        nb_conf_box_neg = tf.reduce_sum(tf.cast(conf_mask_neg > 0.0, dtype=tf.float32))
        nb_conf_box_pos = tf.subtract(tf.cast(total_boxes, dtype=tf.float32), nb_conf_box_neg) #tf.reduce_sum(tf.to_float(conf_mask_pos > 0.0))
        ...
        if self.debug:
            ...
            nb_pred_box = tf.reduce_sum(tf.cast(true_box_conf > 0.32, dtype=tf.float32) * tf.cast(pred_box_conf > 0.3, dtype=tf.float32))

            current_recall = nb_pred_box / (nb_true_box + 1e-6)
            total_recall = tf.compat.v1.assign_add(total_recall, current_recall)

            total_loss = tf.compat.v1.assign_add(total_loss, loss)
            ...
            loss = tf.compat.v1.Print(loss, [loss_xy], message='\nLoss XY \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [loss_wh], message='Loss WH \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [nb_conf_box_neg], message='Nb Conf Box Negative \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [nb_conf_box_pos], message='Nb Conf Box Positive \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [loss_conf_neg], message='Loss Conf Negative \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [loss_conf_pos], message='Loss Conf Positive \t', summarize=1000)
            ...
            loss = tf.compat.v1.Print(loss, [total_loss / seen], message='Average Loss \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [nb_pred_box], message='Number of pred boxes \t', summarize=1000)
            loss = tf.compat.v1.Print(loss, [nb_true_box], message='Number of true boxes \t', summarize=1000)
