...
def detect(save_img=False):
    ...
    ###################### Saving Labels
    data = {"Results": [], "Labels": [], "Red": 0, "Green": 0, "Off": 0, "Total": 0}
    stop = False
            ...
            if view_img:
                ...
                if nframes == 1:
                    ...
                    if cv2.waitKey(1) & 0xFF == ord('q'):  # q to quit
                        print("Average FPS: {}".format(frame/(time.time() - t0)))
                        raise StopIteration
    ...
    with open(os.path.join(os.getcwd(), save_path[:save_path.rfind('.')] + '.json'), 'w+') as outfile:
        json.dump(data, outfile, indent=4)
    
    print('Done. (%.3fs)' % (time.time() - t0))
    print("Average FPS: {}".format(nframes/(time.time() - t0)))
    # cap.release()
